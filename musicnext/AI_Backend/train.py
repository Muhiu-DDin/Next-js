...
#         torch.save(dehaze_net.state_dict(), './saved_models/dehaze_net_epoch_%d.pth' % epoch)
...
def train(orig_images_path, hazy_images_path, batch_size, epochs):
    ...
    criterion = nn.MSELoss().cuda()
    optimizer = optim.Adam(dehaze_net.parameters(), lr=1e-3, weight_decay=1e-4)

    dehaze_net.train()
    for epoch in range(epochs):
        epoch_loss = 0
        for img_orig, img_haze in train_loader:
        ...
        print(f'EPOCH: {epoch+1:02d} LOSS: {epoch_loss / len(train_loader):.3f}')
        torch.save(dehaze_net.state_dict(), f'./saved_models/dehaze_net_epoch_{epoch}.pth')

if __name__ == '__main__':
    train('./data/gt/', './data/hazy/', batch_size=32, epochs=30)
